...
export class EncounterlistComponent implements OnInit {
  ...
  tomorrow = new Date();
  encounterdata: { SstartDate: string; SendDate: string; Checked: any; provider_Id: any; location_Id: any; };
  showEncounterControls: boolean;

  showencounterTable: boolean;
  customizedspinner: boolean;
  EncountersColumns = ["Encounter_Id", "Patient_Name", "Provider_Name", "Birth_Date", "Patient_Age", "Encounter_Date", "Proc_Code",
  ...
  disableApplyButtonEncounterlist(){
    debugger;
    var Provider_Id = this.encounterForm.value.provider_Id == null ? "":this.encounterForm.value.provider_Id;
    var StartDate = this.encounterForm.value.SstartDate == null ? "": this.encounterForm.value.SstartDate;
    var EndDate = this.encounterForm.value.SendDate == null ? "": this.encounterForm.value.SendDate;
  ...
  downloadEncountersExcel(){
    ...
    if(this.encounterlist.data.length != 0){
      this.tableutil.exportAsExcelFileEncounter(this.encounterlist.data, 'Encounter');
    }
  }
}
